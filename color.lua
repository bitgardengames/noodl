...
function Color.withAlpha(color, alpha, options)
        local base = Color.copy(color, options)
        local target = (options and options.target) or {}
        target[1], target[2], target[3] = base[1], base[2], base[3]
        target[4] = (base[4] or 1) * (alpha or 1)
        return target
end
