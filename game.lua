...
function Game:setupFloor(floorNum)
    ...
    local traitContext = {
        ...
        pillars = math.min(math.floor((floorNum + 1) / 4), 5),
    }
    ...
    local numPillars = traitContext.pillars or 0
    ...
    -- Spawn stone pillars before rocks so floor hazards reserve their tiles
        for i = 1, numPillars do
                local fx, fy, col, row = SnakeUtils.getSafeSpawn(Snake:getSegments(), nil, Rocks, safeZone)
                if fx then
                        Pillars:spawn(fx, fy, { col = col, row = row })
                end
        end
