...
local function buildButtons(sw, sh)
	...
	for floor = 1, highestUnlocked do
		local col = (floor - 1) % columns
                local row = mathFloor((floor - 1) / columns)
...
function FloorSelect:enter(data)
	...
	local requestedHighest = data and data.highestFloor
        highestUnlocked = max(1, mathFloor(requestedHighest or PlayerStats:get("deepestFloorReached") or 1))
	local totalFloors = #Floors
	if totalFloors > 0 then
		highestUnlocked = min(highestUnlocked, totalFloors)
	end

        defaultFloor = max(1, min(highestUnlocked, mathFloor((data and data.defaultFloor) or highestUnlocked)))
