...
local CANDY_FLOOR_TEMPLATE = {
        name = "Candied Canvas",
        flavor = "A sugar rush warps the grove into frosting swirls and chunky pixels.",
        palette = {
                bgColor        = {0.22, 0.08, 0.24, 1},
                arenaBG        = {0.96, 0.72, 0.86, 1},
                arenaBorder    = {0.98, 0.52, 0.66, 1},
                snake          = {0.98, 0.88, 0.48, 1},
                rock           = {0.86, 0.56, 0.72, 1},
                sawColor       = {0.98, 0.74, 0.84, 1},
                sprinkleColor  = {0.98, 0.9, 0.58, 1},
        },
        backgroundEffect = {
                type = "candyCascade",
                backdropIntensity = 0.88,
                arenaIntensity = 0.52,
        },
        backgroundTheme = "festival",
        backgroundVariant = "candy",
        screenEffect = {
                type = "pixelate",
                pixelSize = 6,
                blend = 1.0,
...
Floors.easterEggChance = 0.05

function Floors:shouldTriggerEasterEgg(_)
        if not love or not love.math or not love.math.random then
                return false
        end

        local chance = self.easterEggChance or 0
        if chance <= 0 then
                return false
        end
...
function Floors:buildEasterEggFloor(floorNum)
        local template = CANDY_FLOOR_TEMPLATE
        if not template then
                return nil
        end

        local floor = deepCopy(template)
        floor.baseFloor = floorNum
        return floor
end
