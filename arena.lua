...
function Arena:rebuildTileDecorations()
        ...
        local function makeClusterColor()
                ...
                local amount = lighten and (0.12 + rng:random() * 0.12) or (0.18 + rng:random() * 0.12)
                local alpha = lighten and (0.08 + rng:random() * 0.04) or (0.1 + rng:random() * 0.04)
                local color = mixColorTowards(baseColor, target, amount, alpha)

                if rng:random() < 0.2 then
                        local accentMix = 0.2 + rng:random() * 0.15
                        local accentAlpha = clamp01((color[4] or 1) * (0.8 + rng:random() * 0.2))
        ...
        local function jitterColor(color)
                ...
                jittered[4] = clamp01((jittered[4] or 1) * (0.9 + rng:random() * 0.12))
                return jittered
        end
