...
function FloorPlan.buildBaselineFloorContext(floorNum)
    ...
    context.laserCount = math.max(0, math.min(5, (lastPlan.laserCount or 2) + math.floor(extraFloors / 6)))
    context.dartCount = math.max(0, math.min(5, (lastPlan.dartCount or 2) + math.floor(extraFloors / 7)))
    applyLaserCap(context)
    applyDartCap(context)
